...
\* \begin{ppcal}  (* literate-programming marker; commented for SANY/tlapm *)
...
(*                                                                         *)
(* The `shiftR' and `bitshift' helpers below are unused by the algorithm   *)
(* itself (the spec uses `idx' -> `rescale').  We leave them as commented  *)
(* alternative-implementation notes so that `tlapm', which does not        *)
(* support RECURSIVE operator declarations, can parse this module.         *)
(***************************************************************************)
